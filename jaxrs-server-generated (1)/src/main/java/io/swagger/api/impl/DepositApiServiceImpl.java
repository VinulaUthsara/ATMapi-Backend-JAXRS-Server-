...
public class DepositApiServiceImpl extends DepositApiService {
    @Override
    public Response pOSTDeposit( @Min(1)Integer accountNumber,  @Min(1)Integer pin,  @Min(1)Integer depositAmount, SecurityContext securityContext) throws NotFoundException {
       ...
       Map<Integer, AccountDetails> accountMap = Bootstrap.getMap();
           ...
           if(accountMap.get(accountNumber) instanceof AccountDetails) {
               if(accountMap.get(accountNumber).getAccount_pin()== pin ){
                   ...
                   account.setAccount_balance(account.getAccount_balance()+depositAmount);
